  ...
  const SetLaneletRoute::Request::SharedPtr req, const SetLaneletRoute::Response::SharedPtr res)
{
  ...
  std::optional<TransformStamped> transform_to_map_frame;
  try {
  ...
  if (!transform_to_map_frame) {
    set_fail_response(
      ...
      "Failed to transform the pose to the map frame.");
    return;
  }

  change_state(is_reroute ? RouteState::REROUTING : RouteState::ROUTING);
  const auto route = create_route(*req, *transform_to_map_frame);
  ...
  if (is_reroute && is_autonomous_driving) {
    const auto reroute_safety_error = check_reroute_safety(*current_route_, route);
    if (reroute_safety_error.has_value()) {
      RCLCPP_ERROR(get_logger(), "%s", reroute_safety_error->c_str());
      cancel_route();
  ...
  const SetLaneletRoute::Request & req, const TransformStamped & transform)
{
  ...
  return create_route(header, segments, goal_pose, uuid, allow_goal_modification, transform);
}
  ...
  const SetWaypointRoute::Request & req, const TransformStamped & transform)
{
  ...
  return create_route(
    header, waypoints, odometry_->pose.pose, goal_pose, uuid, allow_goal_modification, transform);
}
  ...
  const UUID & uuid, const bool allow_goal_modification, const TransformStamped & transform)
{
  ...
  route.goal_pose = transform_pose(goal_pose, transform);
  ...
  const TransformStamped & transform)
{
  ...
  points.push_back(transform_pose(goal_pose, transform));
...
std::optional<std::string> MissionPlanner::check_reroute_safety(
  const LaneletRoute & original_route, const LaneletRoute & target_route)
{
  ...
  // owns the odometry, so guard it here to keep the original observable behavior (same error
  // message and early return when odometry or map is not yet available).
  if (!map_ptr_ || !lanelet_map_ptr_ || !odometry_) {
    return "Check reroute safety failed. Route, map or odometry is not set.";
  }
