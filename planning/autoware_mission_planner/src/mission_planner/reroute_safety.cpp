  ...
  const double reroute_time_threshold, const double minimum_reroute_length)
{
  ...
  if (!start_idx_opt.has_value()) {
    return "Check reroute safety failed. Cannot find the start index of the route.";
  }
  ...
  if (!ego_is_on_first_target_section) {
    return "Check reroute safety failed. Ego is not on the first section of target route.";
  }
  ...
  if (!start_segment_length.has_value()) {
    return "Check reroute safety failed. Cannot find the closest lanelet.";
  }
  ...
  return "Length of lane where original and B target (= " + std::to_string(accumulated_length) +
