...
FilterResult MessageFilter::apply_filter(
  const nav_msgs::msg::Odometry::SharedPtr input) const
{
  Vector3D linear_velocity{input->twist.twist.linear.x, input->twist.twist.linear.y, input->twist.twist.linear.z};
  Vector3D angular_velocity{input->twist.twist.angular.x, input->twist.twist.angular.y, input->twist.twist.angular.z};
...
: rclcpp::Node("stop_filter", node_options),
  message_filter_(
    declare_parameter<double>("vx_threshold"),
    declare_parameter<double>("wz_threshold"))
{
